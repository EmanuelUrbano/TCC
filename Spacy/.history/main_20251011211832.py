...
def extract_section_body(file_path: str, section_title_pattern: str) -> dict or None:
    """
    ...
    try:
        ...
        compiled_pattern = re.compile(section_title_pattern, re.IGNORECASE)

        for span in doc.spans["layout"]:
            print(f"Regex: '{compiled_pattern}'")
            print(f"Titulo encontrado: '{span.text}'")

            titulo_extraido = span.text
            titulo_corrigido = titulo_extraido.replace("˙","ç")

            if span.label_ == "section_header" and compiled_pattern.search(titulo_corrigido):
                in_target_section = True
                print(f"Seção '{titulo_corrigido}' encontrada. Extraindo conteúdo...")
                continue

            if in_target_section:
                if span.label_ == "section_header" and not compiled_pattern.search(titulo_corrigido):
        ...
        if not in_target_section:
            print(f"Seção com o padrão '{section_title_pattern}' não encontrada no arquivo: {file_path}")
            return None
        else:
            return {
...
if __name__ == "__main__":
    ...
    palavras_chave = ["hipertensão","hipertensões"]
    palavras_chave2 = ["diabete","diabetes"]
    ...
    if not arquivos:
        ...
        for document_path in arquivos:
            ...
            extracted_content = extract_section_body(document_path, title_regex)
            ...
            if extracted_content:
                ...
                if any(palavra.lower() in lemmas_texto for palavra in palavras_chave):                    
                    destino = "diabetes"
                elif any(palavra.lower() in lemmas_texto for palavra in palavras_chave):                    
                    destino = "hipertensao"
                else:
                    destino = "outros"
            else:
